...
RESULT_ROOT = f'{BASE_PATH}/result_rf'
...
def build_rf_params(params):
    return {
        'n_estimators': N_ESTIMATORS,
        'criterion': params['criterion'],
        'max_depth': normalize_optional_int(params['max_depth']),
        'min_samples_split': int(params['min_samples_split']),
        'min_samples_leaf': int(params['min_samples_leaf']),
        'max_features': normalize_optional_str(params['max_features']),
...
def compute_cv_scores(X, y, params, use_upsample):
    ...
    for fold_index, (train_idx, valid_idx) in enumerate(splitter.split(X, y), start=1):
        ...
        if use_upsample:
            sampler = RandomOverSampler(random_state=RF_RANDOM_STATE)
            X_train, y_train = sampler.fit_resample(X_train, y_train)

        model = RandomForestClassifier(**build_rf_params(params))
...
def suggest_rf_params(trial, search_space):
...
def save_model_config(config, model_dir):
    config_rows = [
        ...
        {'item': 'rf_random_state', 'value': RF_RANDOM_STATE},
        {'item': 'n_estimators', 'value': N_ESTIMATORS},
    ]
...
def prepare_candidate_df(candidate_df):
    ...
    prepared_df['criterion_rank'] = prepared_df['criterion'].map({'gini': 0, 'entropy': 1})
    prepared_df['class_weight_key'] = prepared_df['class_weight'].astype(str).replace({'nan': 'None'})
    prepared_df['class_weight_rank'] = prepared_df['class_weight_key'].map({'balanced': 0, 'None': 1})
    prepared_df['max_features_key'] = prepared_df['max_features'].astype(str).replace({'nan': 'None'})
    prepared_df['max_features_rank'] = prepared_df['max_features_key'].map({'sqrt': 0, 'log2': 1, 'None': 2})

    # Prefer a simpler forest among candidates within one standard error.
    prepared_df['sort_max_samples'] = -prepared_df['max_samples']
    prepared_df['sort_min_samples_leaf'] = -prepared_df['min_samples_leaf']
    prepared_df['sort_min_samples_split'] = -prepared_df['min_samples_split']
...
def fit_final_model(X_train, y_train, best_params, use_upsample):
    ...
    model = RandomForestClassifier(**build_rf_params(best_params))
...
def run_single_model(config):
    ...
    best_within_one_row = candidate_df.sort_values(
        [
            'sort_max_samples',
            'sort_min_samples_leaf',
            'sort_min_samples_split',
            'max_depth',
            'max_features_rank',
            'criterion_rank',
            'class_weight_rank',
    ...
    best_params = {
        'criterion': best_within_one_row['criterion'],
        'max_depth': best_within_one_row['max_depth'],
        'min_samples_split': best_within_one_row['min_samples_split'],
        'min_samples_leaf': best_within_one_row['min_samples_leaf'],
        'max_features': best_within_one_row['max_features'],
        'max_samples': best_within_one_row['max_samples'],
    ...
    final_params_df = pd.DataFrame({
        'parameter': list(build_rf_params(best_params).keys()),
        'value': list(build_rf_params(best_params).values()),
    })
    ...
    final_params_df.to_csv(f'{model_dir}/final_rf_params.csv', index=False)
    ...
    return {
        ...
        'selected_criterion': build_rf_params(best_params)['criterion'],
        'selected_max_depth': build_rf_params(best_params)['max_depth'],
        'selected_min_samples_split': build_rf_params(best_params)['min_samples_split'],
        'selected_min_samples_leaf': build_rf_params(best_params)['min_samples_leaf'],
        'selected_max_features': build_rf_params(best_params)['max_features'],
        'selected_max_samples': build_rf_params(best_params)['max_samples'],
        'selected_class_weight': build_rf_params(best_params)['class_weight'],
    }
