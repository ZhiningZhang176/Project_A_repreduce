...
RESULT_ROOT = f'{BASE_PATH}/result_tree'
...
def compute_cv_scores(X, y, params, use_upsample):
    ...
    for fold_index, (train_idx, valid_idx) in enumerate(splitter.split(X, y), start=1):
        ...
        if use_upsample:
            sampler = RandomOverSampler(random_state=TREE_RANDOM_STATE)
            X_train, y_train = sampler.fit_resample(X_train, y_train)

        model = DecisionTreeClassifier(**build_tree_params(params))
...
def suggest_tree_params(trial, search_space):
...
def roc_auc_objective_factory(X, y, use_upsample, search_space):
    def objective(trial):
        params = suggest_tree_params(trial, search_space)
...
def save_model_config(config, model_dir):
    config_rows = [
        ...
        {'item': 'tree_random_state', 'value': TREE_RANDOM_STATE},
    ]
...
def prepare_candidate_df(candidate_df):
    ...
    # Prefer a simpler tree among candidates within one standard error.
    prepared_df['sort_ccp_alpha'] = -prepared_df['ccp_alpha']
    prepared_df['sort_min_impurity_decrease'] = -prepared_df['min_impurity_decrease']
...
def fit_final_model(X_train, y_train, best_params, use_upsample):
    ...
    model = DecisionTreeClassifier(**build_tree_params(best_params))
...
def run_single_model(config):
    ...
    best_params = {
        ...
        'min_impurity_decrease': best_within_one_row['min_impurity_decrease'],
        'ccp_alpha': best_within_one_row['ccp_alpha'],
    ...
    final_params_df = pd.DataFrame({
        'parameter': list(build_tree_params(best_params).keys()),
        'value': list(build_tree_params(best_params).values()),
    })
    ...
    final_params_df.to_csv(f'{model_dir}/final_tree_params.csv', index=False)
    ...
    return {
        ...
        'selected_criterion': build_tree_params(best_params)['criterion'],
        'selected_max_depth': build_tree_params(best_params)['max_depth'],
        'selected_min_samples_split': build_tree_params(best_params)['min_samples_split'],
        'selected_min_samples_leaf': build_tree_params(best_params)['min_samples_leaf'],
        'selected_min_impurity_decrease': build_tree_params(best_params)['min_impurity_decrease'],
        'selected_ccp_alpha': build_tree_params(best_params)['ccp_alpha'],
        'selected_class_weight': build_tree_params(best_params)['class_weight'],
    }
